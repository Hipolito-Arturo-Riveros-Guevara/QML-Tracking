...
// Función para seleccionar aleatoriamente n elementos de un vector
vector<TripletResult> seleccionarAleatoriamente(const vector<TripletResult>& tripletes, int n, unsigned seed) {
    if (tripletes.size() <= n) {
        return tripletes;  // Si hay menos de n, devolver todos
    }
    
    vector<TripletResult> resultado;
    vector<size_t> indices(tripletes.size());
    
    // Llenar con índices
    for (size_t i = 0; i < tripletes.size(); i++) {
        indices[i] = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        resultado.push_back(tripletes[indices[i]]);
    }
...
int main() {
    // Semilla para números aleatorios (usando tiempo actual)
    unsigned seed = chrono::system_clock::now().time_since_epoch().count();
    
    // Leer datos
    cout << "Leyendo archivo entrada.csv..." << endl;
    vector<Hit> data = readCSV("entrada.csv");
    ...
    vector<TripletResult> resultados_finales;
    int tripletes_totales = 0;
    const int MAX_TRIPLETES_POR_CAPA = 500;
    
    cout << "\nIniciando procesamiento de tripletes..." << endl;
    cout << "Máximo de tripletes por capa: " << MAX_TRIPLETES_POR_CAPA << endl;
    
    // Procesar por capas
    for (int l = 1; l <= 8; l++) {  // Ahora hasta 8 como en el código original
        cout << "\nProcesando capa " << l << "..." << endl;
        ...
        // Vector para almacenar todos los tripletes válidos de esta capa
        vector<TripletResult> tripletes_capa;
        int tripletes_encontrados = 0;
                    ...
                    for (const auto& hit3 : hits_layer2) {
                        ...
                        if (abs((phi3 - phi2) / (r3 - r2)) <= 6e4 && abs(z00) <= 100) {
                            ...
                            if (theta_break <= 0.05 && phi_break <= 0.05) {
                                ...
                                tripletes_capa.push_back(result);
                                tripletes_encontrados++;
                                
                                if (tripletes_encontrados % 20000 == 0) {
                                    cout << "  Progreso - Tripletes encontrados en capa " << l 
                                         << ": " << tripletes_encontrados << endl;
                                }
                            }
                        }
                    }
        ...
        cout << "  - Tripletes válidos encontrados en capa " << l << ": " << tripletes_encontrados << endl;
        
        // Seleccionar aleatoriamente máximo 500 tripletes
        vector<TripletResult> tripletes_seleccionados = seleccionarAleatoriamente(
            tripletes_capa, MAX_TRIPLETES_POR_CAPA, seed + l);
        
        cout << "  - Tripletes seleccionados aleatoriamente: " << tripletes_seleccionados.size() << endl;
        
        // Agregar al resultado final
        resultados_finales.insert(resultados_finales.end(), 
                                tripletes_seleccionados.begin(), 
                                tripletes_seleccionados.end());
        
        tripletes_totales += tripletes_seleccionados.size();
        
        cout << "  - Tripletes totales acumulados: " << tripletes_totales << endl;
    }
    
    // Escribir resultados
    cout << "\nEscribiendo resultados..." << endl;
    writeCSV(resultados_finales, "resultado.csv");
    cout << "=== PROCESAMIENTO COMPLETADO ===" << endl;
    cout << "Total de tripletes en el resultado: " << tripletes_totales << endl;
